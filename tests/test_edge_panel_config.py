...
class EdgePanelConfigTests(unittest.TestCase):
    ...
    def test_panel_logs_include_colored_diagnostics(self):
        panel = load_panel_module()
        logs = panel.collect_panel_logs()
        html = PANEL_PATH.read_text(encoding="utf-8")

        self.assertIn("diagnostics", logs)
        self.assertIn("hardware", logs)
        ...
        self.assertEqual(panel.diagnostic_severity_from_text("ui_global_error: boom"), "error")
        self.assertEqual(panel.diagnostic_severity_from_text("warning: cache stale"), "warning")
        self.assertIn("diagnostic-item", html)
        self.assertIn("Hardware diagnostics", html)
        self.assertIn("Runtime parameters", html)
        self.assertIn("function logSeverityFromText", html)
        self.assertIn("log-severity", html)
        self.assertIn("log-warning", html)
        self.assertIn("strongestSeverity(panelLogs.diagnostics", html)
    ...
    def test_nvr_timeline_does_not_rerender_during_active_playback(self):
        ...
        self.assertIn("recorded_day_stream", html)
        self.assertIn("server_cache_tail_day_stream_select", html)
        self.assertIn("ui_recording_cache_tail_day_stream_resume", html)
        self.assertNotIn("server_cache_tail_file_select", html)
        self.assertNotIn("cache_tail_file_resume", html)
        ...
        self.assertIn("ui_recording_day_stream_resume", html)
        self.assertIn("function isRecordedDayStreamMode", html)
        self.assertIn("function recordingVideoDiagnostics", html)
        self.assertIn("ui_recording_video_error", html)
        self.assertIn("timeline_start=", html)
        self.assertIn("mode=playback", html)
        ...
        self.assertIn("recording_thumbnail_warmup_per_camera", html)
        ...
        self.assertIn("diag-ok", html)
        self.assertIn("ui_global_error", html)
        ...
        self.assertIn("nvrGrid.addEventListener('seeking'", html)
        self.assertIn("nvrGrid.addEventListener('seeked'", html)
        self.assertIn("ui_nvr_interaction_protected", html)
        self.assertIn("protected_playback", html)
    ...
    def test_recording_status_exposes_video_timeline_metadata(self):
        ...
        self.assertEqual(status["timeline"]["mode"], "day_wall_clock")
        self.assertEqual(status["timeline"]["total_seconds"], 86400)
        self.assertEqual(status["timeline"]["day_total_seconds"], 86400)
        self.assertEqual(status["timeline"]["recorded_seconds"], 24)
        self.assertEqual(status["timeline"]["playback_total_seconds"], 24)
        self.assertEqual(files[0]["kind"], "video_segment")
        self.assertEqual(files[0]["duration_seconds"], 12)
        self.assertIn("timeline_offset", files[0])
        self.assertIn("playback_offset", files[0])
        self.assertEqual(files[0]["playback_offset"], 12)
        self.assertEqual(files[1]["playback_offset"], 0)
        self.assertIn("recording-thumbs/", files[0]["thumbnail_url"])
        self.assertIn("?v=", files[0]["thumbnail_url"])
        ...
        self.assertIn("?v=", status["days"][0]["thumbnail_url"])
    ...
    def test_recording_status_can_select_calendar_day(self):
        ...
        self.assertEqual(status["timeline"]["total_seconds"], 86400)
        self.assertEqual(status["timeline"]["recorded_seconds"], 10)
